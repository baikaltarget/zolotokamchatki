...
export function generateStaticParams() { return pool("zamorozka").map((p) => ({ slug: p.slug })); }
export async function generateMetadata({ params }: { params: P }) {
  const pr = await params; const p = pool("zamorozka").find((x) => x.slug === pr.slug); if (!p) return {};
...
export default async function Page({ params }: { params: P }) {
  const pr = await params; const p = pool("zamorozka").find((x) => x.slug === pr.slug); if (!p) notFound();
