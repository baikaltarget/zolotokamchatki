    ...
    <Link href={productPath(p)} className="group block bg-white rounded-tag border border-ivory2 hover:border-ink/40 transition-colors overflow-hidden h-full">
      {withImage && (
        <div className="relative aspect-[4/3] bg-ink2">
          ...
          {p.hit && !p.preorder && <span className="absolute right-3 top-3 bg-caviar text-white font-display uppercase text-[11px] tracking-wider px-2 py-1 rounded-tag">Хит</span>}
          {p.preorder && <span className="absolute right-3 top-3 bg-gold text-ink font-display uppercase text-[11px] tracking-wider px-2 py-1 rounded-tag">Под заказ</span>}
        </div>
      )}
      <div className="p-4">
